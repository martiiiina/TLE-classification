...
for fold, (train_idx, test_idx) in enumerate(kf.split(all_subjects, all_labels)):
    ...
    # Modello
    model = SFCN(output_dim=1)
    model.to(device)

    #Caricamento pesi pre-addestrati
    #fp="run_20190719_00_epoch_best_mae.p"
    #pretrain=torch.load(fp, map_location=device)
    ...
    pretrain_weights = pretrain  # Usa direttamente il dict come state_dict
    
    # Rimuovi eventualmente il prefisso 'module.' dai nomi dei layer
    renamed_dict = {k.replace('module.', ''): v for k, v in pretrain_weights.items()}

    # Prende solo i pesi compatibili
    pretrain_dict = {k: v for k, v in renamed_dict.items() 
